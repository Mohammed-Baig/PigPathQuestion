...
int main()
{
	...
	if (water_r > barn_r)
	{
		...
		pig_number = calculatepath(barn_c, barn_r, water_c, water_r);
   	}
	...
	else if (water_r < barn_r)
	{
		...
		pig_number = calculatepath(water_c, water_r, barn_c, barn_r);
	}
...
int calculatepath(int barn_c, int barn_r, int water_c, int water_r)
{
	water_r--;
